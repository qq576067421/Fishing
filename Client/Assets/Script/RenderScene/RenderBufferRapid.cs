...
namespace Ps
{
    public class CRenderBufferRapid : CRenderBuffer
    {
        //-------------------------------------------------------------------------
        float mfTotalSecond = 0.0f;// 状态运行总秒数
        const float mfFireTimeSpan = 0.1f;// 本地发炮间隔秒数
        ...
        public override void destroy()
        {
            ...
            if (_isMe())
            {
                mScene.getProtocol().c2sEndRapid(mScene.getMyPlayerId());
            }

            //mScene.getListener().onSceneShowInfo("结束极速发炮");
        }
